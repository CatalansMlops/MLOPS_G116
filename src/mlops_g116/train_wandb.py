...
def train(lr: float = 0.001, batch_size: int = 32, epochs: int = 5) -> None:
    ...
    for epoch in range(epochs):
        ...
        for i, (img, target) in enumerate(train_dataloader):
            ...
            if i % 100 == 0:
                ...
                images = [wandb.Image(im.detach().cpu(), caption=f"Input {j}") for j, im in enumerate(img[:5])]
    ...
    torch.save(model.state_dict(), "model.pth")  ##caldria guardar arxiu dins run de wandb (i definitiu en models/) !!
